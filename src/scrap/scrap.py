...
from src.scrap.scrap_method import scrap_allfor, process_wivity_batch, scrap_thinkGood, process_linkareer_batch
...
async def scrap_service():
    ...
    print("============= Scrap Start =============")
    print(f"[Scrap][{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Start Scraping")
    ...
    wevitiy_url_list = [URL1, URL2, URL3]

    linkareer_urls = [
        "https://linkareer.com/list/contest?filterBy_categoryIDs=35&filterBy_targetIDs=3&filterBy_targetIDs=4&filterType=TARGET&orderBy_direction=DESC&orderBy_field=CREATED_AT",
        "https://linkareer.com/list/contest?filterBy_categoryIDs=35&filterBy_targetIDs=3&filterBy_targetIDs=4&filterType=TARGET&orderBy_direction=DESC&orderBy_field=CREATED_AT&page=2"
    ]
    
    for j in [URL1, URL2, URL3]:
        wevitiy_url_list.append(j + f"&gp=2")
    ...
    async with aiohttp.ClientSession(
        ...
        page_task1 = [
            process_wivity_batch(session, url, semaphore) for url in wevitiy_url_list
        ]
        page_task2 = [process_linkareer_batch(session, url, semaphore) for url in linkareer_urls]

        page_task3 = [scrap_thinkGood(session, get_driver()), scrap_allfor(session)]
